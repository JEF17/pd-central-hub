  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-4xl px-6 py-10">
        ...
        {session?.portalCharacters?.length ? (
          <section className="mt-4 rounded-xl border border-border bg-card p-4">
            <div className="flex items-center gap-2">
              <Users className="size-4 text-primary" />
              <h2 className="text-sm font-semibold">UCP Karakterlerin</h2>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              Bir karaktere tıkladığında o karakter için ayrı bir personel profili açılır ve adı
              soyadı otomatik doldurulur.
            </p>
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {session.portalCharacters.map((c) => {
                const fullName = `${c.firstname} ${c.lastname}`.trim();
                const linked = profiles.find(
                  (p) => p.characterId === c.id || (!p.characterId && p.name.trim() === fullName),
                );
                return (
                  <button
                    key={c.id}
                    ...
                    onClick={() => {
                      if (linked) {
                        persist(
                          profiles.map((p) =>
                            p.id === linked.id ? { ...p, characterId: c.id } : p,
                          ),
                          linked.id,
                        );
                        notify.success(`${fullName} profiline geçildi`);
                        return;
                      }
                      const fresh: StoredOfficerProfile = {
                        ...createEmptyStoredProfile(),
                        name: fullName,
                        characterId: c.id,
                      };
                      persist([...profiles, fresh], fresh.id);
                      notify.success(`${fullName} için profil oluşturuldu`);
                    }}
                    className={`flex items-center gap-3 rounded-lg border p-3 text-left transition hover:border-primary/60 hover:bg-primary/5 ${
                      linked && linked.id === activeId
                        ? "border-primary bg-primary/10"
                        : "border-border"
                    }`}
                  >
                    <span className="flex size-9 items-center justify-center overflow-hidden rounded-md bg-muted/50">
                      {linked?.photo ? (
                        <img src={linked.photo} alt={fullName} className="size-full object-cover" />
                      ) : (
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="block text-xs text-muted-foreground">
                        {linked ? linked.rank || "Profil hazır" : "Profil oluştur"}
                      </span>
                    </span>
                  </button>
                );
